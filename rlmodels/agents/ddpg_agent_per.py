...
class Actor(keras.Model):
    def __init__(self, state_size, action_size, units):
        ...
        self.hidden_layers.append(keras.layers.Dense(action_size, activation='tanh'))
...
class Critic(keras.Model):
    ...
    def call(self, states_actions):
        x = tf.concat(states_actions, axis=-1)
        for layer in self.hidden_layers:
            x = layer(x)
...
class DDPGAgent():
    def __init__(self, state_size, action_size, action_limits, config, seed=None):
        ...
        self.action_mag = (self.action_high - self.action_low) / 2

        self.mse = keras.losses.MeanSquaredError()
        self.actor_optimizer = keras.optimizers.Adam(1e-4)
        self.critic_optimizer = keras.optimizers.Adam(1e-3)
    ...
    def act(self, state, noisy=True):
        ...
        action = action * self.action_mag + self.action_mean
    ...
    def learn(self, experiences):
        ...
        self.critic_optimizer.apply_gradients(
            zip(critic_grads, self.critic_local.trainable_variables))
